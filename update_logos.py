...
for relative_path in files_to_update:
    ...
    try:
        ...
        new_content = new_content.replace('className="h-24 w-auto"', 'className="h-20 w-auto"')
        
        # Also handle the case where class might be slightly different or in footer with extra classes
        # But based on grep, "h-24" seems consistent. 
        # In footer of page.tsx it is "mb-4 h-24 w-auto".
        new_content = new_content.replace('className="mb-4 h-24 w-auto"', 'className="mb-4 h-20 w-auto"')
